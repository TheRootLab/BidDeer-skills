...
The caller imports the core modules, implements `LLMProviderAdapter`, and injects it into `ReasoningEngine`.
...
```bash
python -m core.cli retrieve --csv checklist.csv --docx proposal.docx --out candidates.json
...
1. Run `python -m core.cli retrieve` with a CSV checklist and DOCX proposal to write `candidates.json`.
...
4. Run `python -m core.cli report` with `candidates.json` and `judgments.json` to write the Markdown report.
...
- You would need to modify `core/**`, add dependencies, add console script entrypoints, or change CLI/runtime behavior while only performing release documentation updates.
